...
extern "C" const char *duckdb_vx_agg_func_name(duckdb_vx_agg_func ffi) {
    D_ASSERT(ffi);
    return reinterpret_cast<BoundAggregateFunction *>(ffi)->GetName().c_str();
}
...
extern "C" duckdb_vx_expr_class duckdb_vx_expr_get_class(duckdb_vx_expr ffi_expr) {
    ...
    auto expr = reinterpret_cast<Expression *>(ffi_expr);
    const auto expr_class = expr->GetExpressionClass();
    if (expr_class == ExpressionClass::BOUND_FUNCTION) {
        // DuckDB 2.0 represents casts, comparisons, and betweens as bound
        // functions; map them back to the classes the FFI consumer expects.
        if (BoundCastExpression::IsCast(*expr)) {
            return DUCKDB_VX_EXPR_CLASS_BOUND_CAST;
        }
    ...
    return static_cast<duckdb_vx_expr_class>(expr_class);
}

extern "C" duckdb_logical_type duckdb_vx_expr_get_return_type(duckdb_vx_expr ffi_expr) {
    D_ASSERT(ffi_expr);
    auto expr = reinterpret_cast<Expression *>(ffi_expr);
    // const_cast required: FFI hands out a borrowed pointer, no mutable accessor exists.
...
extern "C" const char *duckdb_vx_expr_get_bound_column_ref_get_name(duckdb_vx_expr ffi_expr) {
    ...
    auto str = expr.GetName().GetIdentifierName();
    auto result = static_cast<char *>(duckdb_malloc(str.size() + 1));
    memcpy(result, str.c_str(), str.size() + 1);
    return result;
}
