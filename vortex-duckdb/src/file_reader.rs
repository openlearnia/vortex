...
impl OpenFileReader {
    async fn open(path: String, encryption_key: Option<SegmentEncryptionKey>) -> VortexResult<Self> {
        let (fs, fs_path) = resolve_filesystem(&parse_uri_or_path(&path)?)?;
        let source = fs.open_read(&fs_path).await?;
        let file = open_cached(&SESSION, Some(&path), source, None, &|mut options| {
            if let Some(key) = &encryption_key {
                options = options.with_encryption_key(key.clone());
            }
            options
        })
        .await?;
...
pub fn reader_open(file_path: &str) -> VortexResult<OpenFileReader> {
    reader_open_with_key(file_path, None)
}

/// Open a file for scanning, optionally decrypting segments with a raw AES-GCM key
/// (DuckLake `encryption_key` / `encryption_config.footer_key_value`).
pub fn reader_open_with_key(
    file_path: &str,
    encryption_key: Option<Vec<u8>>,
) -> VortexResult<OpenFileReader> {
    let key = encryption_key
        .as_deref()
        .filter(|bytes| !bytes.is_empty())
        .map(SegmentEncryptionKey::try_new)
        .transpose()?;
    RUNTIME.block_on(OpenFileReader::open(file_path.to_owned(), key))
}
