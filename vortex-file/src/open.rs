...
impl VortexOpenOptions {
    ...
    ///
    /// `path` is the object's *literal* key, exactly as the store holds it — a key containing
    /// `~`, `%`, `#`, `[`, `]`, and friends is used verbatim rather than percent-encoded. See
    /// [`object_path_from_literal`](vortex_io::object_store::object_path_from_literal).
