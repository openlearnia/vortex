...
/// Regression for #9420: a resolved key must survive all the way to the store.
///
/// The registry resolves a URL to the literal key, but every later `&str` -> `Path` hop used to
/// re-encode it (`~` -> `%7E`) *as part of the key*, and the request layer then encoded the
/// resulting `%` again (`%7E` -> `%257E`). The read landed on a key no object has and the store
/// 404'd. Resolving alone cannot catch that — only opening the file walks those hops — so this
/// drives the whole chain: seed the store with the literal key out of band, resolve the URL, then
/// open through `vortex-file`.
///
/// The payload is not a real Vortex file, so the open fails either way; what distinguishes the bug
/// is *how*. Reaching the footer parse proves the request found the object, whereas a re-encoded
/// key never gets that far and comes back `NotFound`. (Writing a real file here would need an
/// edition-enabled session, which only the `vortex` umbrella crate can build — and it depends on
/// this crate.)
#[rstest::rstest]
